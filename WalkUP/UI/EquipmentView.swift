...
/// 設定（画面 #9）。権限・デモモード・復元。
///
/// **この実装は作り直す（2026-07-27 決定）。**
/// 当初は「設定だけは OS の作法に従う方が迷わない」として標準の `Form` を残していたが、
/// その判断は取り消した。**開いた瞬間に iOS の灰色のリストが出れば、
/// 他の画面で作った世界は一枚で消える。** 迷わせない責任は部品の出自ではなく、
/// 行の構造と言葉が負う。切り替えは同じ挙動を持つ自前の部品で作り、
/// 並びと余白だけ標準に合わせればよい。
///
/// 作り直しの見本は `web/ui.js` の `settingsScreen()` と `web/README.md`。
/// 直近7日は数の一覧ではなく棒を添える（歩いた日と歩かなかった日が形で分かる）。
struct SettingsView: View {
    ...
    var body: some View {
        ...
        // ここが標準の `Form` のままなのは、**まだ作り直していないから**。
        // 意匠を揃える対象として残っている（上のコメント参照）。
    }
}
