...
### 記録の ID を変えると、既存プレイヤーの記録が消える

12体を実装したとき図鑑の ID を役割ベース（`zako_ch1_standard`）から
種別ベース（`darari`）に変えた。**この時点では図鑑画面が無かったので誰も気付かなかった。**
図鑑を作って初めて、討伐済みのダラリが「未発見」に戻っていることが見えた。
`GameState.migratingLegacyBestiaryIDs()` で読み込み時に読み替えている。
**保存データに入る文字列を変えるときは、必ず移行を同じコミットで書くこと。**

### キャンバス内の「大きさ」はプロンプトで指定できない

ボスは STYLE SPEC を 70% → 85% に上げて生成したが、実測すると縦占有は 69%・46% で
雑魚（70%）と変わらなかった。**大きさは表示側のフレームで作ること**
（`BattleView` のボスは 310pt、雑魚は 170pt）。詳細は `ART_PROMPTS.md` §3.6 の表。
